...
function TaskListItem({ task, onComplete }: { task: Task; onComplete: (id: string) => void }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl px-3 py-2.5 group hover:bg-white/5 transition-colors duration-200">
      <button
    ...
    <div className="glass-panel p-6 shadow-md transition-all duration-300 hover:shadow-lg">
      ...
      {myTasks.length === 0 ? (
        ...
        <div className="space-y-1">
          {myTasks.map((task) => (
            <TaskListItem key={task.id} task={task} onComplete={(id) => updateTaskStatus(id, 'done')} />
          ))}
        </div>
      )}
    </div>
